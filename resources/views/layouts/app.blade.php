...
<head>
    ...
    {{-- PWA runtime availability is controlled centrally by FeatureAccessService. --}}
    @if($pwaAvailable)
    ...
    <style>:root{--acad-primary:{{ $primaryColor }};--acad-primary-rgb:{{ $primaryR }} {{ $primaryG }} {{ $primaryB }};}</style>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-2xl p-4">
            <div class="flex items-start justify-between gap-4 mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-semibold text-sm">Install {{ \App\Services\SettingService::get('site_name', 'AcadFlow') }}</h3>
        ...
        <header class="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-slate-200">
            <div class="flex items-center justify-between px-4 py-3">
                ...
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2">
                    ...
                    <span class="font-bold text-lg text-slate-950">{{ $siteName }}</span>
                </a>
            ...
            <aside id="sidebar" class="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0 lg:left-0 lg:z-30 bg-white border-r border-slate-200">
                <div class="px-5 py-5 border-b border-slate-100">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5">
                        ...
                        <h1 class="text-lg font-black tracking-tight text-slate-950">{{ $siteName }}</h1>
                    </a>
                </div>
                ...
                <div class="p-3 border-t border-slate-100">
                    <button type="button" onclick="toggleUserDropdown()" class="flex w-full items-center gap-3 rounded-xl px-2 py-2 text-left transition hover:bg-slate-50">
                        <span class="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-200 text-xs font-bold text-slate-600">
                            @if(auth()->user()?->avatar_media_id && auth()->user()?->avatarMedia)
                                <img src="{{ route('media.preview', auth()->user()->avatarMedia) }}" alt="" class="h-full w-full object-cover">
                        ...
                        <span class="min-w-0 flex-1"><span class="block truncate text-xs font-bold text-slate-900">{{ auth()->user()?->full_name ?? auth()->user()?->first_name ?? 'User' }}</span><span class="block truncate text-[10px] capitalize text-slate-500">{{ str_replace('_',' ',auth()->user()?->role ?? 'member') }}</span></span>
            ...
            <main class="flex-1 lg:pl-64 min-w-0">
                <div class="px-4 py-5 sm:px-6 lg:px-7">
                    ...
                    <div class="hidden lg:flex items-center justify-between mb-5">
                        <div>
                            <h2 class="text-xl font-black tracking-tight text-slate-950">@yield('page-title', 'Dashboard')</h2>
                            ...
                            <div class="relative">
                                ...
                                >
                                    <div class="h-8 w-8 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden">
                                        @if(auth()->user()?->avatar_media_id && auth()->user()?->avatarMedia)
                                            <img src="{{ route('media.preview', auth()->user()->avatarMedia) }}" alt="{{ auth()->user()->full_name }} profile photo" class="h-full w-full object-cover">
                                        @elseif(auth()->user()?->avatar)
                                            <img src="{{ asset('storage/' . auth()->user()->avatar) }}" alt="{{ auth()->user()->full_name }} profile photo" class="h-full w-full object-cover">
                                        @else
                                ...
                                >
                                    @if($notificationsVisible)
                                    <a href="{{ route('notifications.settings') }}" class="block px-4 py-3 text-sm text-slate-700 hover:bg-slate-50">
                                        Notification Preferences
                                    </a>
                                    @endif

                                    @if(Route::has('knowledge.creator.edit'))
                                        <a href="{{ route('knowledge.creator.edit') }}" class="block px-4 py-3 text-sm text-slate-700 hover:bg-slate-50">
                                            My Profile
                                        </a>
                                    @endif
                    ...
                    @if($featurePreview && auth()->user()?->isAdmin())
                        <div class="mb-5 flex flex-col gap-2 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                {{ $featurePreview['title'] ?? 'Feature' }} is currently
                                <span class="font-semibold">{{ str($featurePreview['status'] ?? 'restricted')->headline() }}</span>
                            ...
                            @if(auth()->user()?->role === 'super_admin' && Route::has('admin.settings.features'))
                                <a href="{{ route('admin.settings.features') }}" class="shrink-0 font-bold underline underline-offset-2">Manage features</a>
                            @endif
                        </div>
                    @endif
