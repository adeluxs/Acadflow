            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($universities as $university)
                    <tr>
                        ...
                        <td class="px-6 py-4">{{ $university->code }}</td>
                        <td class="px-6 py-4">{{ $university->email }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $university->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
...
<div id="createModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <form method="POST" action="{{ route('admin.universities.create') }}">
            @csrf
            <div class="mb-4">
