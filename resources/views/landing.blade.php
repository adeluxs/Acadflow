...
<head>
    ...
    <title>{{ $siteName }} — Academic Workflow Platform</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <style>
        :root{--acad-primary:{{ $primaryColor }};--acad-primary-rgb:{{ $primaryR }} {{ $primaryG }} {{ $primaryB }};}
...
<body class="landing-glow min-h-screen text-slate-950 antialiased">
    <header class="relative z-20">
        <nav class="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8" aria-label="Main navigation">
            <a href="{{ url('/') }}" class="flex items-center gap-3" aria-label="{{ $siteName }} home">
            ...
            <div class="flex items-center gap-3">
                @auth
                    <a href="{{ route('dashboard') }}" class="hidden px-3 py-2 text-sm font-semibold text-slate-800 sm:inline-flex">Dashboard</a>
                    <a href="{{ route('dashboard') }}" class="acad-primary-button !rounded-lg !px-6">Open AcadFlow</a>
                @else
                    <a href="{{ route('login') }}" class="hidden px-3 py-2 text-sm font-semibold text-slate-800 sm:inline-flex">Log in</a>
                    <a href="{{ $registerUrl }}" class="acad-primary-button !rounded-lg !px-6">Get Started</a>
                @endauth
            </div>
        </nav>
    </header>
        ...
        <section class="mx-auto grid max-w-7xl gap-12 px-5 pb-8 pt-7 lg:grid-cols-[.88fr_1.2fr] lg:items-center lg:px-8 lg:pb-4 lg:pt-7">
            <div class="relative z-10 pb-3">
                ...
                <div class="mt-7 flex flex-wrap gap-3">
                    <a href="{{ $registerUrl }}" class="acad-primary-button !rounded-xl !px-6 !py-3.5">Get Started Free <span class="ml-2">→</span></a>
                ...
                <div class="mt-7 flex items-center gap-4">
                    <div class="flex -space-x-2">
                        @foreach(['IB','AM','TK','SA','DM'] as $avatar)
                            <span class="flex h-9 w-9 items-center justify-center rounded-full border-2 border-white bg-slate-800 text-[9px] font-bold text-white">{{ $avatar }}</span>
                        @endforeach
                    </div>
            ...
            <div class="relative lg:-mr-8">
                    ...
                    <div class="grid min-h-[535px] grid-cols-[158px_1fr] sm:grid-cols-[178px_1fr]">
                        <aside class="border-r border-slate-100 bg-white p-4">
                            ...
                            <div class="mt-4 space-y-1 text-[10px] font-semibold text-slate-600">
                                @foreach([['Dashboard','⌂'],['Courses','▯'],['Assignments','▤'],['Attendance','▣'],['Discussions','▢'],['AI Assistant','✦'],['Gradebook','▦'],['Reports','⌁'],['Billing','▣'],['Settings','⚙']] as $i=>$item)
                                    <div class="flex items-center gap-2 rounded-lg px-2.5 py-2.5 {{ $i===0 ? 'bg-indigo-50 text-indigo-700' : '' }}"><span class="w-4 text-center">{{ $item[1] }}</span><span>{{ $item[0] }}</span>@if($item[0]==='AI Assistant')<span class="ml-auto rounded-full bg-indigo-100 px-1.5 py-0.5 text-[8px] text-indigo-700">New</span>@endif</div>
                                @endforeach
                            </div>
                        ...
                        <div class="mini-scrollbar overflow-hidden bg-white p-5">
                            ...
                            <div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                                @foreach([['My Courses','8','Active courses','bg-indigo-50 text-indigo-600'],['My Students','248','Total students','bg-emerald-50 text-emerald-600'],['Pending Reviews','32','Submissions','bg-orange-50 text-orange-600'],['Attendance Today','92%','Average rate','bg-blue-50 text-blue-600']] as $stat)
                                    <div class="rounded-xl border border-slate-100 bg-white p-3 shadow-sm"><div class="flex items-start justify-between"><p class="text-[8px] text-slate-500">{{ $stat[0] }}</p><span class="flex h-6 w-6 items-center justify-center rounded-md {{ $stat[3] }} text-[9px]">◈</span></div><p class="mt-3 text-xl font-black">{{ $stat[1] }}</p><p class="mt-1 text-[8px] text-slate-500">{{ $stat[2] }}</p></div>
                                @endforeach
                            </div>
                            <div class="mt-5 grid gap-4 sm:grid-cols-[1.15fr_.85fr]">
                                <div class="rounded-xl border border-slate-100 p-4 shadow-sm"><div class="flex items-center justify-between"><p class="text-[10px] font-black">Recent Submissions</p><span class="text-[8px] font-bold text-indigo-600">View all</span></div><div class="mt-3 space-y-2">@foreach([['Data Structures Assignment','CSC301 · 15 submissions','Pending','bg-amber-50 text-amber-600'],['SIWES Report - Week 8','IND401 · 23 submissions','In Review','bg-blue-50 text-blue-600'],['Final Year Project Chapter 3','CSC499 · 8 submissions','Completed','bg-emerald-50 text-emerald-600']] as $row)<div class="flex items-center gap-2 rounded-lg border border-slate-100 p-2.5"><span class="flex h-7 w-7 items-center justify-center rounded-md bg-indigo-50 text-indigo-600 text-[9px]">▤</span><div class="min-w-0 flex-1"><p class="truncate text-[9px] font-bold">{{ $row[0] }}</p><p class="text-[8px] text-slate-400">{{ $row[1] }}</p></div><span class="rounded-full px-2 py-1 text-[7px] font-semibold {{ $row[3] }}">{{ $row[2] }}</span></div>@endforeach</div></div>
                                <div class="relative overflow-hidden rounded-xl border border-indigo-100 bg-gradient-to-br from-white to-indigo-50 p-4 shadow-sm"><div class="absolute -right-3 -top-4 h-16 w-16 rounded-full bg-indigo-200/50 blur-xl"></div><div class="relative"><div class="flex items-center justify-between"><div><p class="text-[10px] font-black">✦ AI Assistant</p><p class="mt-1 text-[8px] text-slate-500">How can I help you today?</p></div><span class="flex h-12 w-12 items-center justify-center rounded-full bg-slate-950 text-[9px] font-bold text-white">AI</span></div><div class="mt-3 space-y-1.5">@foreach(['Review a submission','Check plagiarism','Generate feedback','Analyze document'] as $tool)<div class="rounded-md bg-white px-2.5 py-2 text-[8px] font-medium shadow-sm">{{ $tool }}</div>@endforeach</div><div class="mt-3 rounded-md bg-indigo-600 px-3 py-2 text-center text-[8px] font-bold text-white">Start with AI Assistant ↗</div></div></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div id="features" class="relative -mt-1 grid grid-cols-3 gap-px overflow-hidden rounded-b-[22px] border border-indigo-100 bg-slate-100 shadow-xl shadow-indigo-100/40 sm:grid-cols-6">
                    @foreach([['✦','AI Academic','Assistant','bg-violet-50 text-violet-600'],['▤','Smart Assignment','Management','bg-emerald-50 text-emerald-600'],['▦','Attendance','Tracking','bg-blue-50 text-blue-600'],['⌕','Plagiarism','Detection','bg-orange-50 text-orange-600'],['⌁','Analytics &','Reports','bg-violet-50 text-violet-600'],['▣','Knowledge','Hub','bg-emerald-50 text-emerald-600']] as $feature)
                        <div class="bg-white px-2 py-5 text-center"><span class="mx-auto flex h-8 w-8 items-center justify-center rounded-lg {{ $feature[3] }} text-xs">{{ $feature[0] }}</span><p class="mt-2 text-[9px] font-semibold leading-3 text-slate-800">{{ $feature[1] }}<br>{{ $feature[2] }}</p></div>
                    @endforeach
                </div>
            </div>
        </section>
            ...
            <div class="rounded-3xl border border-slate-100 bg-white/90 p-6 shadow-lg shadow-indigo-50/70">
                ...
                <div class="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    @foreach([
                        ...
                        <article class="flex gap-3 rounded-xl border border-slate-200 p-4 transition hover:border-indigo-200 hover:shadow-sm"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600">@if($role[2]==='cap')◈@elseif($role[2]==='user')♙@elseif($role[2]==='building')▥@else◇@endif</span><div><p class="text-xs font-black">{{ $role[0] }}</p><p class="mt-1 text-[10px] leading-4 text-slate-500">{{ $role[1] }}</p></div><span class="ml-auto text-slate-400">→</span></article>
                    @endforeach
                </div>
            </div>
        </section>
